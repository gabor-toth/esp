...
void rest_send_message_back( httpd_req_t *req, const char *format, ... ) {
    int size = snprintf( NULL, 0, "{ \"message\": \"%s\" }", format );
    size++;
    char *full_format = malloc( size );
    snprintf( full_format, size, "{ \"message\": \"%s\" }", format );

    va_list args;
    va_start ( args, format );
    size = vsnprintf( NULL, 0, full_format, args );
    va_end ( args );
    size++;
    char *response = malloc( size );
    va_start ( args, format );
    vsnprintf( response, size, full_format, args );
    va_end ( args );

                                    ...
                                    int32_t event_id, void* event_data)
{
    ...
    {
        initial_scan = false;
        power_led_set_mode(POWER_LED_MODE_WIFI_SCAN);
    }
    wifi_scan();
}

static void start_rescan_timer(bool _initial_scan)
{
    initial_scan = _initial_scan;
    int timeout;
    if (initial_scan)
    {
        wifi_shutdown();
    ...
    xTimerChangePeriod(reconnect_timer, pdMS_TO_TICKS( timeout * 1000 ), portMAX_DELAY);
...
esp_err_t wifi_main(const char* default_hostname)
{
    ...
    reconnect_timer = xTimerCreate(
        "wifiReconnect",
        pdMS_TO_TICKS(CONNECT_TIMEOUT_SECS * 1000),
        0,
        NULL,
        on_reconnect_timer);
    ESP_ERROR_CHECK(

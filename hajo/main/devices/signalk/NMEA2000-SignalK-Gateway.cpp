...
void HandleGNSS( const tN2kMsg &N2kMsg ) {
    ...
    if ( ParseN2kGNSS( N2kMsg, SID, DaysSince1970, SecondsSinceMidnight, Latitude, Longitude, Altitude, GNSSType,
        ...
        time_t unix_time =  (long) ( DaysSince1970 * 24L * 60 * 60 + SecondsSinceMidnight );
        struct tm ts;
...
//    }
    ...
    // animation
    SecondsSinceMidnight += TICK_PERIOD/1000.0;
    tick++;
}
...
void pngSimulationStart() {
    ...
    ESP_ERROR_CHECK(esp_timer_start_periodic( tick_timer, TICK_PERIOD*1000 ));
}

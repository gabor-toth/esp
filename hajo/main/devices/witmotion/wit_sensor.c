...
_Noreturn static void process_task(void *pvParameters) {
    ...
    for (;;) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            // see https://www1.grc.nasa.gov/beginners-guide-to-aeronautics/aircraft-rotations/
            fAngle[ i ] = (float) sReg[ Roll + i ] / 32768.0f * 180.0f;
            if (i == 0) {
                // roll: left - right, negative: roll left, positive: roll right
                fAngle[ i ] = fAngle[ i ] + 90.0f;
            } else if (i == 1) {
                // pitch: forward - backward, negative: pitch forward, positive: pitch backwards
                fAngle[ i ] = -fAngle[ i ];
            } else if (i == 2) {
                // yaw: heading
                // horizontal mounting: 180 - x
                // vertical mounting:   ?
                fAngle[ i ] = 180.f - fAngle[ i ];
            }
        }
...
static bool findSensor() {
    ...
    while (!found) {
        for (int i = 0; i < sizeof(c_uiBaud) / sizeof(c_uiBaud[ 0 ]) && !found; i++) {

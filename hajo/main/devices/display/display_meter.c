...
void setup_bar_common( int index, lv_obj_t *bar, lv_style_t *style_indic, int y0 ) {
    ...
    lv_obj_set_size( bar, STYLE_BAR_WIDTH - 2 * STYLE_BAR_OUTLINE,
                     STYLE_BAR_HEIGHT + 2 * STYLE_BAR_PADDING - 2 * STYLE_BAR_OUTLINE );
...
lv_obj_t *setup_fluid_label( lv_obj_t *parent, int y0 ) {
    ...
    for ( int i = 0; i < 5; i++ ) {
        ...
        lv_obj_set_pos( label, STYLE_BAR_X + STYLE_BAR_PADDING + i * (STYLE_BAR_WIDTH - 3 * STYLE_BAR_PADDING) / 4 - i,
                        y0 + 2 );
...
void display_meter_main() {
    ...
    prev = bar_battery[ 0 ] = setup_battery_bar( screen,
                                                 obj_get_bottom( prev ) + 2 * STYLE_BAR_HEIGHT + STYLE_BAR_OUTLINE, 0 );
    setup_battery_label( screen, obj_get_bottom( prev ) + STYLE_BAR_OUTLINE );
    prev = bar_battery[ 1 ] = setup_battery_bar( screen, obj_get_bottom( prev ) + STYLE_BAR_HEIGHT + STYLE_BAR_OUTLINE,
                                                 1 );
    setup_battery_label( screen, obj_get_bottom( prev ) + STYLE_BAR_OUTLINE );
